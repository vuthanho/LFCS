function values = compute_colorstabilization_aff( I1, I1tmp, I2tmp, coef, clip_v, gamma_ref, use_sift  )
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%
...
%% otherwise, it means there is not enough information
...
Hl = -100.*ones(4); Hl(1,1) = 1e-2; Hl(2,2) = 1e-2; Hl(3,3) = 1e-2; Hl(4,4) = 1e-2;
...
[gammas1, fval1 ] = fmincon( @(x) myfunction11D4_aff(x,  I1_reshape, I2_reshape  ),  [gamma0 gamma_ref H0(:)'], [], [], [], [], [min_gamma gamma_ref Hl(:)'], [max_gamma gamma_ref Hu(:)'], [], options );
[gammas2, fval2 ] = fmincon( @(x) myfunction11D5_aff(x,  I1_reshape, I2_reshape  ),  [gamma0 gamma_ref H0(:)'], [], [], [], [], [min_gamma gamma_ref Hl(:)'], [max_gamma gamma_ref Hu(:)'], [], options );

disp(['fval1 ', num2str(fval1), ', fval2 ', num2str(fval2)])

%% Select the better optimization
if fval1 > fval2
    gammas = gammas2;
    H2 = [reshape(gammas2(3:end), 3 , 4); 0 0 0 1]; 
    H1 = pinv(H2);
else
    gammas = gammas1;
    H1 = [reshape(gammas1(3:end), 3 , 4);0 0 0 1];
    H2 = pinv(H1);
end
...
output = apply_colorstabilization_aff( I1, I1, matrix, gammas );
...
% 
% max_clip = ( H1 * ( (clip_v(2).^gammas(1)) .* ones(4,1) ) );
% min_clip = ( H1 * ( (clip_v(1).^gammas(1)) .* ones(4,1) ) );
% 
% values.max_clip = max_clip(1:3); %./max_clip(4);
% values.min_clip = min_clip(1:3); % ./min_clip(4);

max_clip = ( H1 * ( [(clip_v(2).^gamma(1)) .* ones(3,1) ; 1]) );
min_clip = ( H1 * ( [(clip_v(1).^gamma(1)) .* ones(3,1) ; 1]) );
...
% end
% axis([0 1 0 1 0 1])
% output = apply_colorstabilization_aff( pts1, pts2, matrix, gammas );
% 
