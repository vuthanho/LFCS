function err = myfunction11D4_hom(x, points1, points2 )
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%
%% 
...
%% Compute the points in lab space 
...
% lab_points2 = rgb2lab_pts( (points2) );
% % 
...
for i=1:size(lin1w, 1)
    lin1ws(i) =  sqrt(sum( (lin1w(i, :))./maxlin1 )); % ( (max(abs(lin1(i, :) - points2(i, :))) - mean_lin1ws + .0) ) .* 
    lin2ws(i) =  sqrt(sum( (lin2w(i, :))./maxlin2 )); %  ( (max(abs(lin2(i, :) - points1(i, :))) - mean_lin2ws + .0) ) .* 
end
...
% maxlab2 = 1;
% for i=1:size(lin1w, 1)
%     lab_lin1s(i) = sqrt(sum( (lab1w(i, :))./maxlab1 )); %  ( abs(max(abs(lab_lin1(i, :) - lab_points2(i, :))) - mean_lab1ws + .0) ) .* 
%     lab_lin2s(i) = sqrt(sum( (lab2w(i, :))./maxlab2 )); %  ( abs(max(abs(lab_lin2(i, :) - lab_points1(i, :))) - mean_lab2ws + .0) ) .* 
% end
% 
...
if( H1(1, 1) < 0 || H1(2, 2) < 0 || H1(3, 3) < 0 ...
 || H2(1, 1) < 0 || H2(2, 2) < 0 || H2(3, 3) < 0 || p2~=0 || rank(H2)<size(H2, 1) || p1~=0 || rank(H1)<size(H1, 1) ) % ...  || p1~=0 || rank(H1)<size(H1, 1)  || p2~=0 || rank(H2)<size(H2, 1)
    err = 1e14;
    return
end